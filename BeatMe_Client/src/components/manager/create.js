    ...
    <div className="create-style">
      ...
      <div className="create-props">
        <input type="text" placeholder={placeholder_name}
          id="name" value={cname}
          onChange={event=> {setCname(event.target.value); localStorage.setItem("compName",event.target.value)}} className="competition-name-input" ></input><br />
        <span className='error'>{cnameError}</span>
        <div className="card-center">
        <div className="card-type-competition" >
          {images.map((image) => {
            return <button className={image.path === buttonSelected ? "create-button-selected" : "select-competition-type"} 
